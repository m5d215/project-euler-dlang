/// Problem 11: 格子内の最大の積
///
/// 20x20 の格子のうち、上下左右斜めのいずれかの方向で連続する 4 つの数字の積のうち最大のものはいくつか。
module project_euler.problem011;

import system.linq : aggregate, block, counting, maximum, select, Yield;
...
{
    enum continuation = 4;
    enum delta = [1:0, 0:1, 1:1, -1:1];
    static void iterationBlock(Yield!uint yield)
    {
        foreach (x; continuation - 1 .. grid[0].length - continuation)
        foreach (y; continuation - 1 .. grid.length - continuation)
        foreach (dx, dy; delta)
        {
            yield = counting[0 .. continuation].select!(i => grid[y + dy * i][x + dx * i]).aggregate!"a * b";
        }
    }
    return block!iterationBlock.maximum;
}

unittest
{
    assert(problem011() == 70600674);
}
